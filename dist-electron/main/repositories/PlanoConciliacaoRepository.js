...
class PlanoConciliacaoRepository {
    /**
    ...
    async atualizarNomePlano(id, nome) {
        return await (0, localConnection_1.run)("UPDATE planos_conciliacao SET nome_plano = ? WHERE id = ?", [nome, id]);
    }
    async obterPlanoPorId(id) {
        const rows = await (0, localConnection_1.query)("SELECT * FROM planos_conciliacao WHERE id = ?", [
            id,
        ]);
        return rows[0] || null;
    }
    async listarTodosPlanos() {
        return await (0, localConnection_1.query)("SELECT * FROM planos_conciliacao");
    }
    async excluirItensDoPlano(planoId) {
        return await (0, localConnection_1.run)("DELETE FROM itens_plano_conciliacao WHERE plano_id = ?", [
            planoId,
        ]);
    }
    /**
    ...
    async adicionarItem(item) {
        ...
        return await (0, localConnection_1.run)(`INSERT INTO itens_plano_conciliacao 
