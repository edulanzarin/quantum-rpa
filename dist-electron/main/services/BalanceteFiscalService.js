...
class BalanceteFiscalService {
    planoContaService = new PlanoContaService_1.PlanoContaService();
    fiscalService = new LancamentoFiscalEntradaService_1.LancamentoFiscalEntradaService();
    planoConciliacaoRepo = new PlanoConciliacaoRepository_1.PlanoConciliacaoRepository();
    async gerarBalancoPatrimonialFiscal(codigoEmpresa, dataInicio, dataFim, planoConciliacaoId) {
        const arvorePlanoContas = await this.planoContaService.obterPlanoProcessado(codigoEmpresa);
        const notas = await this.fiscalService.buscarLancamentosFiscais(codigoEmpresa, dataInicio, dataFim);
        const regrasItens = await this.planoConciliacaoRepo.obterItensPorPlano(planoConciliacaoId);
        ...
        for (const nota of notas) {
            ...
            if (regra && regra.contabiliza) {
                ...
                const processarImpostoRecuperavel = (valor, contaAtivo) => {
                    if (valor > 0) {
                ...
                processarImpostoRecuperavel(valorCofinsProporcional, CONTA_COFINS_RECUPERAR);
                // 3. Lança DÉBITO PRINCIPAL (Valor Cheio)
                if (regra.contasDebito) {
                    regra.contasDebito.forEach((contaId) => {
                        this.adicionarSaldo(saldosProjetados, contaId, valorParaDebitoPrincipal, "D");
                    });
                }
                // 4. Lança CRÉDITO PRINCIPAL (Fornecedor)
                if (regra.contasCredito) {
                    regra.contasCredito.forEach((contaId) => {
                        this.adicionarSaldo(saldosProjetados, contaId, valorParaCreditoPrincipal, "C");
                    });
                }
                // 5. Lança CRÉDITO RETIDOS
                if (valorParaContaRetidos > 0) {
                    this.adicionarSaldo(saldosProjetados, CONTA_RETIDOS, valorParaContaRetidos, "C");
                }
            }
        }
        // --- GERAÇÃO FINAL ---
        ...
        for (const raiz of arvorePlanoContas) {
            if (raiz.CLASSIFCONTA.startsWith("1") ||
    ...
    adicionarSaldo(mapa, contaId, valor, tipo) {
    ...
    acumularSaldosHierarquicos(node, saldosMap, resultado, nivelMaximo) {
        let debito = 0;
        let credito = 0;
        ...
        for (const filho of node.filhos) {
            const saldoFilho = this.acumularSaldosHierarquicos(filho, saldosMap, resultado, nivelMaximo);
